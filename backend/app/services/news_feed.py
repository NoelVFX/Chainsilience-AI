...
Returns the news that actually matters to a company, in priority order:
  ...
  3. and — so the panel is never blank — the latest news as a fallback when the
     company has no twin yet or nothing relevant is found.
"""
...
def relevant_news(session, company_id: int, limit: int = 6) -> list[NewsItem]:
    ...
    # 3. Never leave the panel blank.
    if not picked:
        picked = news_repo.latest(limit)
    return picked
